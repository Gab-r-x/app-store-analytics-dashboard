...
interface Props {
  data: { category: string; revenue: number | null }[]
}
...
export default function RevenueChart({ data }: Props) {
  const cleanData = data.filter(item => typeof item.revenue === "number")
  const maxValue = Math.max(...cleanData.map(item => item.revenue || 0))
